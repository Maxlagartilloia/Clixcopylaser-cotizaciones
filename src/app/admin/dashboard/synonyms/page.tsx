...
import { getSynonyms, addSynonym, deleteSynonym } from '@/app/actions';
...
export default function SynonymsPage() {
    ...
    const handleAddSynonym = async (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        setIsSubmitting(true);
        const formData = new FormData(event.currentTarget);
        const newSynonym: Omit<Synonym, 'id'> = {
            term: formData.get('term') as string,
            normalizedTerm: formData.get('normalizedTerm') as string,
        };

        try {
            await addSynonym(newSynonym);
            toast({ title: 'Éxito', description: 'Sinónimo añadido correctamente.' });
            fetchSynonyms();
            setIsDialogOpen(false);
        } catch (error) {
            toast({ variant: 'destructive', title: 'Error', description: 'No se pudo añadir el sinónimo.' });
        } finally {
            ...
            <div className="flex justify-between items-center mb-8">
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogTrigger asChild>
                        <Button><PlusCircle className="mr-2 h-4 w-4" />Añadir Sinónimo</Button>
                    </DialogTrigger>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Añadir Nuevo Sinónimo</DialogTitle>
                            <DialogDescription>
                        ...
                        <form onSubmit={handleAddSynonym} className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <Input id="term" name="term" placeholder="ej: cuaderno 100h lineas" required />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="normalizedTerm">Término Canónico (del Catálogo)</Label>
                                <Input id="normalizedTerm" name="normalizedTerm" placeholder="ej: cuaderno 100 hojas rayado" required />
                            </div>
                            <DialogFooter>
